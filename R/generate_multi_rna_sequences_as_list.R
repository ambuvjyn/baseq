#' Generate multiple RNA sequences as a list
#'
#' @param num_seq The number of rna sequences to generate.
#' @param seq_length The length of each rna sequence.
#' @return A list of rna sequences.
#' @examples
#' generate_multi_rna_sequences_as_list(10, 50)
#'
#' @export
generate_multi_rna_sequences_as_list <- function(num_seq, seq_length) {
  seqs <- sapply(paste0("seq_", 1:num_seq), function(seq_name) {
    rna_seq <- paste0(sample(c("A", "C", "G", "U"), seq_length, replace=TRUE), collapse="")
    list(rna_seq)
  })
  names(seqs) <- paste0("seq_", 1:num_seq)
  return(seqs)
}
